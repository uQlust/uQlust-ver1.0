    ...
    public class FragBagProfile:ContactProfile
    {
        ...
        public override void CheckIfAvailable()
        {
            if (!Directory.Exists(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) +
                    Path.DirectorySeparatorChar+"fragBag"))
                if (!Directory.Exists("C:\\Projects\\UQlast\\fragBag"))
                    throw new Exception("Directory fragbag not exists. Profile fragbag cannot be used!");
...
//if (!test && item.Contains("1vij"))
        ...
        private void ThreadLibrary(object o)
        {
            ...
            chunk = new float[fragBagLibrary[0].GetLength(0), fragBagLibrary[0].GetLength(1)];
