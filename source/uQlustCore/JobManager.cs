...
namespace uQlustCore
{
    ...
    public delegate void StartJob(string name,string al,string dirName,DistanceMeasures measure);
    ...
    public class JobManager
    {
        ...
        private void RunHashDendrog(string name, string dirName, string alignmentFile = null, DCDFile dcd = null)
        {
            ...
            if (dcd != null)
                hk = new HashCluster(dcd, opt.hierarchical.hash);
            else
                if (alignmentFile != null)
                    hk = new HashCluster("", alignmentFile, opt.hierarchical.hash);
                else
                    hk = new HashCluster(dirName, null, opt.hierarchical.hash);
